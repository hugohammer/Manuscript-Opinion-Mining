...
\begin{align}
  ...
  +&(1 - \alpha) \cdot \dfrac{\mathbf{sentsc}(sw_{j})}{\mathbf{depdist}(kw_i, sw_{j})}    
  \end{split}
\end{align}
...
\begin{equation}
  \label{eq:4}
  \mathbf{imp}(kw_i, sw_{j}) = \dfrac{\mathbf{sentsc}(sw_{j})}{\mathbf{depdist}(kw_i, sw_{j})}    
\end{equation}
